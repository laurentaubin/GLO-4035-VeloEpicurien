...
class RouteService:
    ...
    def find_starting_point(self, length: int, types: List[str]):
        return {
            "startingPoint": self.__route_repository.find_starting_point(length, types)[
                "geometry"
            ]
        }
    ...
    def generate_route(
        ...
        return self.__route_repository.find_route(
            starting_point, types, length, number_of_stops
        )

...
class NeoGraphRepository:
    ...
    def save_vertexes(self, vertexes: List[Vertex]) -> None:
        ...
        for vertex in vertexes:
            ...
            vertex_node = Node(
                "Vertex",
                latitude=str(vertex_coordinate.get_latitude()),
                longitude=str(vertex_coordinate.get_longitude()),
                segment_id=str(vertex.get_id()),
            )
        ...
        for index, _ in enumerate(vertex_nodes):
            if index != len(vertex_nodes) - 1:
                ...
                distance = self.__calculate_distance_between_vertex(
                    vertexes[index], vertexes[index + 1]
                )
                self.__save_vertexes_relationship(
                    first_vertex_node, second_vertex_node, distance
                )

    def __save_vertexes_relationship(
        self, first_vertex_node: Node, second_vertex_node: Node, distance: float
    ):
        vertex_relationship = Relationship(
            first_vertex_node, "link_to", second_vertex_node, distance=distance
        )
    ...
    def __calculate_distance(
        ...
        a = (
            0.5
            - cos(
                (second_coordinates.get_latitude() - first_coordinates.get_latitude())
                * p
            )
            / 2
            + cos(first_coordinates.get_latitude() * p)
            * cos(second_coordinates.get_latitude() * p)
            * (
                1
                - cos(
                    (
                        second_coordinates.get_longitude()
    ...
    def __calculate_distance_between_vertex(
        self, first_vertex: Vertex, second_vertex: Vertex
    ):
        ...
        a = (
            ...
            - cos(
                (
                    second_vertex_coordinate.get_latitude()
                    - first_vertex_coordinate.get_latitude()
                )
                * p
            )
            / 2
            + cos(first_vertex_coordinate.get_latitude() * p)
            * cos(second_vertex_coordinate.get_latitude() * p)
            * (
                1
                - cos(
                    (
                        second_vertex_coordinate.get_longitude()
                        - first_vertex_coordinate.get_longitude()
                    )
                    * p
                )
            )
    ...
    def __fetch_segment_vertexes_by_id(self, segment_id: str):
        return self.__graph_client.run(
            "MATCH (vertex: Vertex {segment_id: '"
            + str(segment_id)
            + "'}) RETURN vertex"
        )

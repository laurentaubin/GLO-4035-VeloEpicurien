...
class GraphService:
    ...
    def connect_near_segments_together(self) -> None:
        ...
        segments: List[Segment] = self.__segment_repository.find_all()
        for segment in segments:
            number_of_fetch_saved += self.__graph_repository.connect_vertexes(
                segment.get_segment_id(), segment.get_near_segments()
            )
            print(
    ...
    def connect_restaurants_to_segments(self) -> None:
        ...
        for restaurant in restaurants:
            ...
            print(
                "\n RESTAURANT {name} CONNECTED - {nbr_connected}/{total}".format(
                    name=restaurant.get_name(),
                    nbr_connected=str(number_of_restaurants_connected),
                    total=str(total_number_of_restaurants),
                )
            )
